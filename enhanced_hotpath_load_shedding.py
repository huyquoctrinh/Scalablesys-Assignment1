#!/usr/bin/env python3
"""
Hot Path Detection (OpenCEP, no PredicateCondition) + Load Shedding
...
- Load shedding presets: off | semantic | adaptive (latency-aware)
"""
...
# --- Load shedding (optional; mirrors your previous code) ---
LOAD_SHEDDING_AVAILABLE = True
try:
    from loadshedding import (
        LoadSheddingConfig,
        PresetConfigs,                  # optional in some repos; we don't depend on it
...
def enrich_importance_fields(p: dict):
    ...
    # Simple station importance baseline (no global popularity available here)
    station_importance = 0.5
    if p.get("start_station_id") in DEFAULT_TARGETS:
        # proximity proxy: starts from target-ish station
...
def build_load_shedding_config(ls_mode: str, latency_ms: int):
    """
    Mirror your earlier configs:
      - off: None (disabled)
      - semantic: strategy_name='semantic' + pattern priorities + attribute names
      - adaptive: strategy_name='adaptive' + latency thresholds
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Hot Path Detection (OpenCEP, no PredicateCondition) + Load Shedding')
    ...
    # Build CEP with optional load shedding
    ls_config = build_load_shedding_config(FLAGS["ls_mode"], FLAGS["ls_latency_ms"])
    if ls_config and getattr(ls_config, "enabled", False):
        cep = CEP(patterns, load_shedding_config=ls_config)
        print(f'[HotPath] Load Shedding: {FLAGS["ls_mode"]} '
              f'({"%d ms" % FLAGS["ls_latency_ms"] if FLAGS["ls_mode"]=="adaptive" else ""})')
    else:
        cep = CEP(patterns)
        print('[HotPath] Load Shedding: off')
    ...
    # Print LS statistics if supported
    if hasattr(cep, "is_load_shedding_enabled") and cep.is_load_shedding_enabled():
        if hasattr(cep, "get_load_shedding_statistics"):
            ...
            print("[HotPath] LS enabled (no stats method exposed).")
