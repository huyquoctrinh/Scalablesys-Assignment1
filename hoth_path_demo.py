...
def create_enhanced_patterns():
    ...
    pattern1 = Pattern(
        ...
        timedelta(minutes=15)  # 30 minutes timeout
    )
...
class SimpleOutputStream(OutputStream):
    def __init__(self, file_path: str = "hothpath_citybike.txt", is_async: bool = False):
        super().__init__()
        ...
        if self.__is_async:
            print(f"Opening output file for writing: {self.file_path}")
            self.__output_file = open(self.file_path, 'w', buffering=1)  # Line buffered
        else:
            self.__output_file = None
            self.__output_buffer = []
    
    def add_item(self, item: object):
        """Add item with immediate writing."""
        self.matches.append(item)
        print(f"Match found: {item}")
        super().add_item(item)
        
        # Write immediately in both modes
        if self.__is_async and self.__output_file:
            self.__output_file.write(f"MATCH: {item}\n")
            self.__output_file.flush()  # Force immediate write
            print(f"Written match to file: {self.file_path}")
        else:
            self.__output_buffer.append(item)
            # Also write to file immediately for debugging
            with open(self.file_path, 'a') as f:
                f.write(f"MATCH: {item}\n")
            print(f"Buffered and written match to: {self.file_path}")
        
        # ... rest of pattern analysis code ...
    
    def close(self):
        """Enhanced close with better error handling."""
        print(f"Closing output stream. Matches collected: {len(self.matches)}")
        
        if self.__is_async and self.__output_file:
            try:
                self.__output_file.write(f"\n# SUMMARY: {len(self.matches)} total matches\n")
                self.__output_file.close()
                print(f"Closed async output file: {self.file_path}")
            except Exception as e:
                print(f"Error closing async file: {e}")
        elif not self.__is_async and self.__output_buffer:
            try:
                with open(self.file_path, 'w') as f:
                    f.write(f"# CitiBike Hot Path Analysis Results\n")
                    f.write(f"# Generated: {datetime.now()}\n")
                    f.write(f"# Total matches: {len(self.__output_buffer)}\n\n")
                    
                    for i, item in enumerate(self.__output_buffer):
                        f.write(f"Match {i+1}: {item}\n")
                    
                    f.write(f"\n# Analysis Summary:\n")
                    for pattern_name, stats in self.pattern_stats.items():
                        f.write(f"# {pattern_name}: {stats['count']} matches\n")
                
                print(f"Written {len(self.__output_buffer)} matches to: {self.file_path}")
                self.__output_buffer.clear()
            except Exception as e:
                print(f"Error writing buffered output: {e}")
        
        # Always create a summary file
        try:
            summary_file = self.file_path.replace('.txt', '_summary.txt')
            with open(summary_file, 'w') as f:
                f.write(f"CitiBike Hot Path Detection Summary\n")
                f.write(f"Generated: {datetime.now()}\n")
                f.write(f"Output file: {self.file_path}\n")
...
def run_enhanced_demo(csv_files: Union[str, List[str]], max_events: int = 5000, force_async: bool = False, num_threads: int = 4):
    ...
    for config_name, config in configs.items():
        ...
        output_stream = SimpleOutputStream(f"hothpath_{config_name.replace(' ', '_').lower()}.txt", is_async=is_async)
...
def main():
    """Main function with safety defaults."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Enhanced Multi-threaded CitiBike Load Shedding Demo')
    parser.add_argument('--csv', default='test_data/201306-citibike-tripdata.csv',
                       help='Path to CitiBike CSV file(s)')
    parser.add_argument('--files', nargs='+',
                       help='Multiple CSV files to process')
    parser.add_argument('--events', type=int, default=1000,  # Reduced default
                       help='Maximum number of events to process')
    parser.add_argument('--threads', type=int, default=4,
                       help='Number of threads for parallel processing')
    parser.add_argument('--async', action='store_true',
                       help='Enable async output mode')
    parser.add_argument('--complex-patterns', action='store_true',
                       help='Use complex Kleene closure patterns (memory intensive)')
    ...
    results = run_enhanced_demo(
        ...
        force_async=getattr(args, 'async'),
        num_threads=args.threads,
        # use_safe_patterns=not args.complex_patterns  # Use safe patterns by default
    )
